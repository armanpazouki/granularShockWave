...
double data_out_step =
    50 * time_step;  // 1e-8;       // time interval between data outputs
double visual_out_step =
    50 * time_step;  // 1e-7;     // time interval between PovRay outputs
...
float mu = 0.18f;
float ballDensity =
    2000.0f / geomScale / geomScale / geomScale;  // 1.0f; // kg/m/m/m
float ballMass =
    ballDensity * 4.0f / 3.0f * CH_C_PI * ballRad * ballRad * ballRad;
...
ChVector<> inertia =
    (2.0 / 5.0) * ballMass * ballRad * ballRad * ChVector<>(1, 1, 1);
...
double timeStartPushing =
    0;  // 50*time_step;//3.0e-6; // start time for pushing the wall
...
void AddBalls(ChSystemParallelDEM* sys) {
  ...
  double y = 0.;
  double dimention =
      ballDiam - overlap;  // dimention = length of the triangle's side (it
  ...
  for (int ii = 0; ii < zLay; ii++) {
    ...
    for (int jj = 0; jj < xLay; jj++) {
      ...
      if ((jj == 0) ||
          (jj == xLay - 1))  // first and last column should be fixed
        ifFixed = true;
      // =========================================
...
int main(int argc, char* argv[]) {
  ...
  const int numOfParticles =
      2 * howManyParticles - 1;  // -1 because particle 0 is the same particle
  ...
  for (int ii = 0; ii < howManyParticles - 1;
       ii++) {  // note that number of iterations in the loop changed
    particleID[ii + howManyParticles] =
        (particleNumber[ii + 1] - 1) * xLay + (xLay - 1) / 2;
  }
...
// gl_window.SetRenderMode(opengl::WIREFRAME);
  ...
  while (my_system->GetChTime() <= simulation_time) {
    ...
    if (pushed == false && my_system->GetChTime() >= timeStartPushing) {
      ...
      ball_X =
          ChSharedPtr<ChBody>(my_system->Get_bodylist()->at(0 + pushedBall));
      my_system->Get_bodylist()->at(0 + pushedBall)->AddRef();
      ball_X->SetPos_dt(ball_X->GetPos_dt() + ChVector<>(0, 0, -velocity));
      // ball_X->SetPos_dt(ball_X->GetPos() + ChVector<>(0, velocity*time_step,
      // 0));
    }
#else
// Force excitation
    ...
    {  // projections + information about violation
      ...
      for (int ii = 0; ii < xLay * zLay; ii++) {
        ...
        ball_X->SetRot(ChQuaternion<double>(ball_X->GetRot().e0, 0.0,
                                            ball_X->GetRot().e2, 0.0));
        ball_X->SetPos_dt(ChVector<double>(ball_X->GetPos_dt().x, 0.0,
                                           ball_X->GetPos_dt().z));
        ball_X->SetRot_dt(ChQuaternion<double>(
            -0.5 * ball_X->GetRot().e2 * ball_X->GetWvel_loc().y, 0.0,
            0.5 * ball_X->GetRot().e2 * ball_X->GetWvel_loc().y, 0.0));
      ...
      fprintf(planeViol, "%.14f,%.14f,%.14f,%.14f,\n", planeViolation,
              planeViolation2, planeViolationAfter, planeViolationAfter2);
      fclose(planeViol);
    }
#endif
    ...
    if (my_system->GetChTime() >= data_out_frame * data_out_step) {
      ...
      for (int jj = 0; jj < numOfParticles; jj++) {
        ...
        ChSharedPtr<ChBody> ball_X;
        ball_X =
            ChSharedPtr<ChBody>(my_system->Get_bodylist()->at(particleID[jj]));
        my_system->Get_bodylist()->at(particleID[jj])->AddRef();

        fprintf(rowFile, "%.13f,%.13f,%.13f,%.13f,%.13f,%.13f,%.13f,\n",
                my_system->GetChTime(), ball_X->GetPos().x, ball_X->GetPos().z,
                ball_X->GetRotAngle(), ball_X->GetPos_dt().x,
                ball_X->GetPos_dt().z, ball_X->GetWvel_par().y);
    ...
    if (write_povray_data &&
      ...
      sprintf(MaxVelocity_file, "%s/maxVelocity_%04d.dat", pov_vel_dir.c_str(),
              visual_out_frame + 1);
