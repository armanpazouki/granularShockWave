    ...
    real3* ext_body_torque)                 // [output] body torque (two per contact)
{
  ...
  case SEATTLE:
#define m2mm 1e6//1//1e6
#define m2mm2 1//1e-6// 1
	  ifSeattle = true;

	  if (use_mat_props) {
		   // =========== Below Appropriate Seattle model ===========
			  //double rho1 = 3 * 1e28;
			  //double rho2 = 3 * 1e28;
			  //double C = 1e-77;
			  //double A = CH_C_PI * CH_C_PI * C * rho1 * rho2;//9e-21;//C * rho1 * rho2;
			  //============================================================================
			  //double A = 7.25e-20 * m2mm * m2mm; // J = kg * m^2 / s^2
			  //double D = 0.165e-9 * m2mm; // m
			  //double w = -A / (12 * CH_C_PI * D * D);//-A / (12 * CH_C_PI * delta_n * delta_n); <- model for nonintrusive DEM-P
			  //============================================================================
		  double yOverlap = 0.001 * m2mm2;
			  double E_STAR = 1e9 / m2mm; // kg / m / s^2
			  double R_STAR = 0.25 * m2mm2;
			  //delta_n = delta_n * m2mm2;
			  double w = (2 * E_STAR * sqrt(yOverlap * yOverlap * yOverlap)) / (3 * CH_C_PI * sqrt(R_STAR));
			 // std::cout << "w = " << w << "\n";
			  double sqrt_Rd = sqrt(R_STAR * delta_n);

			  kn = -2 * CH_C_PI * R_STAR * w / delta_n/1e30 + 4.0 / 3.0 * E_STAR * sqrt_Rd; // minus should be in here in front of 2
		  
			  //std::cout << 2 * CH_C_PI * eff_radius[index] * w  << "\t" << 4.0 / 3.0 * E_eff * sqrt_Rd * delta_n << std::endl;
		  
			  kt = 0;//8.0 * G_eff * sqrt_Rd;
			  gn = 0;
			  gt = 0;
		   // =========== Above Appropriate Seattle model ===========

		  //////// =========== Below Messing around with Seattle model ===========
			 // double yOverlap = 1.0;
			 // //double w = 1 / (2 * CH_C_PI * eff_radius[index]);//(2 * sqrt(yOverlap * yOverlap * yOverlap)) / (3 * CH_C_PI * sqrt(eff_radius[index]));
			 // //double sqrt_Rd = sqrt(eff_radius[index] * delta_n);

			 // kn = -1 / delta_n + 1 * sqrt(delta_n); //2 * CH_C_PI * eff_radius[index] * w / delta_n + 4.0 / 3.0 * E_eff * sqrt_Rd; 
			 // // that comment with "minus" is UNTRUE -> minus in front of everything because it has to be positive - untrue you idiot!!!
			 ...
			 // gn = 0;
			 // gt = 0;
		  //////// =========== Above Messing around with Seattle model ===========
